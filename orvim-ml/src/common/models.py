...
from .workflow import EnumWorkflowStatus


Base = declarative_base()

apply_workflow_status = ENUM(
    "complete",
    "in_progress",
    "error",
...
class Workflow(Base):
    ...
    connectors_data: Mapped[List[Dict[str, Any]]] = mapped_column(
        JSONB, server_default="{}"
    )
    transformers_data: Mapped[List[Dict[str, Any]]] = mapped_column(
        JSONB, server_default="{}"
    )
    ...
    status: Mapped[EnumWorkflowStatus] = mapped_column(
        apply_workflow_status, server_default=EnumWorkflowStatus.in_progress
    )
    style_settings: Mapped[str] = mapped_column(nullable=False, server_default="")
    host_permissions: Mapped[List[str]] = mapped_column(
        ARRAY(String, dimensions=1), nullable=True, server_default="{}"
    )
...
class TransformLog(Base):
    ...
    connection_id: Mapped[int] = mapped_column(
        ForeignKey("connection_log.id", ondelete="CASCADE")
    )
